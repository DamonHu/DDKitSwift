...
extension ZXKitFPS: ZXKitPluginProtocol {
    ...
    public func start() {
        ...
        self.start { (fps) in
            ...
            let config = ZXKitButtonConfig(title: "\(fps)FPS", titleColor: UIColor.zx.color(hexValue: 0xffffff), titleFont: UIFont.systemFont(ofSize: 13, weight: .bold), backgroundColor: backgroundColor)
            ZXKit.updateFloatButton(config: config, plugin: self)
        }
    }
}
